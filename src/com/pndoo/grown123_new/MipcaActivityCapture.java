...
public class MipcaActivityCapture extends BaseActivity implements Callback {
	...
	public void handleDecode(Result result, Bitmap barcode) {
		...
		if (TextUtils.isEmpty(resultString)) {
			...
			Log.i("userId", UserUtil.getInstance(this).getUserId());
			params.put("userId", UserUtil.getInstance(this).getUserId());

			params.put("code", resultString);
			tm.createNewTask(new TaskListener() {
				...
				public void onPostExecute(BaseTask task, String errorMsg) {
					...
					if (errorMsg != null) {// 获取数据出现异常
						ActivityUtils.showToastForFail(MipcaActivityCapture.this, "添加书籍失败," + errorMsg);
					} else {
						BookJson bookJson = bookController.getModel();
						if (null == bookJson) {
							ActivityUtils.showToastForFail(MipcaActivityCapture.this, "获取书籍信息失败");
							return;
						} else {
							if (TextUtils.isEmpty(bookJson.getCode())) {
								ActivityUtils.showToastForFail(MipcaActivityCapture.this, "请连接网络进行扫描");
								return;
							}
							if (bookJson.getCode().equals(MyPreferences.SUCCESS)) {
								ActivityUtils.showToastForSuccess(MipcaActivityCapture.this, bookJson.getCodeInfo());
								getApplicationContext().ADD_FLAG = 1;
								getApplicationContext().notice = bookJson.getNotice();
								...
								ActivityUtils.showToastForFail(MipcaActivityCapture.this, "获取书籍信息失败," + bookJson.getCodeInfo());
								return;
							}
						}
					}
				...
				public String onDoInBackground(BaseTask task, MultiValueMap<String, String> param) {
					...
					try {
						errorMsg = bookController.findBook(param);
					} catch (Exception e) {
						e.printStackTrace();
					}
					return errorMsg;
