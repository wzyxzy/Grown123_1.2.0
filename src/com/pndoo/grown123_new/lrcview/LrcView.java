...
public class LrcView extends View implements ILrcView {
	...
	/** 无歌词数据的时候 显示的默认文字 **/
	private static final String DEFAULT_TEXT = "对不起,没有找到对应文档";
	/** 默认文字的字体大小 **/
	private static float SIZE_FOR_DEFAULT_TEXT = 54;
	/** 画高亮歌词的画笔 ***/
	private Paint mPaintForHighLightLrc;
	/** 高亮歌词的默认字体大小 ***/
	private static float DEFAULT_SIZE_FOR_HIGHT_LIGHT_LRC = 54;
	/** 高亮歌词当前的字体大小 ***/
	...
	private static final int DEFAULT_COLOR_FOR_HIGHT_LIGHT_LRC = Color.rgb(18,
			187, 146);
	/** 高亮歌词当前的字体颜色 **/
	...
	/** 其他歌词的默认字体大小 ***/
	private static float DEFAULT_SIZE_FOR_OTHER_LRC = 54;
	/** 其他歌词当前的字体大小 ***/
	...
	private static final int DEFAULT_COLOR_FOR_OTHER_LRC = Color.rgb(137, 137,
			137);
	/** 其他歌词当前的字体颜色 **/
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = minRaw; i <= maxRaw; i++) {
			...
			if (i == mCurRow) {// 画高亮歌词
				...
				// mCurSizeForOtherLrc)*mCurFraction;
...
//					text="　　"+text;
					int cou = Math.round(getWidth()
							/ DEFAULT_SIZE_FOR_HIGHT_LIGHT_LRC) + 1;
//					Log.i("str", getWidth() + "");
					int group=0;
					if (text.length()%cou==0) {
						group = text.length() / cou;
					}else {
						group=text.length() / cou+1;
					}
					...
					for (int j = 0; j < group; j++) {
						// canvas.drawText(text.substring(cou*j,cou*(j+1)),
						// textX, rowY+=mCurSizeForOtherLrc + mCurPadding,
						// mPaintForHighLightLrc);
						str = text.substring(start, end);
						
//						Log.i("str", str);
						canvas.drawText(str, textX, /* rowY */
								rowY += mCurSizeForOtherLrc /*+ mCurPadding*/,
								mPaintForHighLightLrc);
						start = end;
						end = cou * (j + 2) >= text.length()
								? text.length()
								: cou * (j + 2);
					}
				} else {
...
//					mPaintForOtherLrc.setColor(0x1000000 * curAlpha
...
//			}
			...
			case MotionEvent.ACTION_MOVE :
				if (!canDrag) {
					if (Math.abs(event.getRawY() - firstY) > mTouchSlop
							&& Math.abs(event.getRawY() - firstY) > Math
									.abs(event.getRawX() - lastX)) {
						canDrag = true;
	...
	public int getmCurRow() {
		return mCurRow;
	}

	public void setmCurRow(int mCurRow) {
		this.mCurRow = mCurRow;
		if (onSeekToListener != null && mCurRow != -1) {
		...
		if (getScrollY() < 0) {
			smoothScrollTo(0, DURATION_FOR_ACTION_UP);
				...
				* (mCurSizeForOtherLrc + mCurPadding) - mCurPadding) {
			smoothScrollTo((int) (mLrcRows.size()
					* (mCurSizeForOtherLrc + mCurPadding) - mCurPadding),
					DURATION_FOR_ACTION_UP);
		}
		canDrag = false;
...
//					}
	...
	private void smoothScrollTo(int dstY, int duration) {
		...
		mScroller.startScroll(getScrollX(), oldScrollY, getScrollX(), offset,
				duration);
		invalidate();
	}
