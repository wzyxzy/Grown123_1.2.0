...
import com.pndoo.grown123_new.dto.base.AuthorMainContent;
 ...
 *        ------------------------------------------------------------------
 */
public class AuthorCommunicationActivity extends BaseActivity implements OnRefreshListener {
	...
	private AuthorComment comment;
	private List<AuthorMainContent> mainContents;
	private CommonAdapter<AuthorMainContent> adapter;
	...
	private void loadData() {
		...
		tm.createNewTask(new TaskListener() {
			...
			public void onPreExecute(BaseTask task) {
				// DialogUtils.showMyDialog(AuthorCommunicationActivity.this,
				// MyPreferences.SHOW_PROGRESS_DIALOG, null, "请稍后...",
				// null);
			}
			...
			public void onPostExecute(BaseTask task, String errorMsg) {
				...
				if (errorMsg != null) {
					// 获取数据出现异常
					DialogUtils.showMyDialog(AuthorCommunicationActivity.this, MyPreferences.SHOW_ERROR_DIALOG, "出现异常", errorMsg, null);
				} else {
					CommentJson commentJson = commentController.getModel();
					if (null == commentJson || null == commentJson.getCode()) {
						DialogUtils.showMyDialog(AuthorCommunicationActivity.this, MyPreferences.SHOW_ERROR_DIALOG, "数据为空", "获取信息失败", null);
						return;
					} else {
						if (commentJson.getCode().equals("SUCCESS")) {
							comment = commentJson.getAuthorComment();
							Log.i("msg", comment.toString());
							updata();
							return;
						} else if (commentJson.getCode().equals("FAIL")) {
							DialogUtils.showMyDialog(AuthorCommunicationActivity.this, MyPreferences.SHOW_ERROR_DIALOG, "获取失败", commentJson.getCodeInfo(), null);
							return;
						}
					}
				}
			...
			public String onDoInBackground(BaseTask task, MultiValueMap<String, String> param) {
				...
				try {
					errorMsg = commentController.authorComment(param);
				} catch (Exception e) {
					e.printStackTrace();
				}
				return errorMsg;
	...
	private void initViews() {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				Intent intent = new Intent(AuthorCommunicationActivity.this, AuthorDescriptionItemActivity.class);
				String content = mainContents.get(position).getContent();
				intent.putExtra("content", content);
				intent.putExtra("id", mainContents.get(position).getId());
				intent.putExtra("addTime", mainContents.get(position).getAddTime());
				intent.putExtra("title", mainContents.get(position).getTitle());
				startActivity(intent);
			}
		});
		initBroadcast();
	...
	private void updata() {
		if (comment != null) {
			...
			adapter = new CommonAdapter<AuthorMainContent>(this, mainContents, R.layout.communication_item) {

				@Override
				public void convert(ViewHolder helper, AuthorMainContent item, int position) {
